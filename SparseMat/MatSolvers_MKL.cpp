...
// ● MKL Pardisoで解く(非対称)
//=======================================================*/
bool MatSolvers::solveMLKpardiso(const slv_int size0, const SparseMat& matA, double* vecB, double *results){
    const int n = size0;
    /* スパース形式コピー */
    int *ia = new int[n+1];
...
//=======================================================*/
bool MatSolvers::solveMLKpardiso(const slv_int size0, const SparseMatC& matA, dcomplex* vecB, dcomplex* results){
    const int n = size0;
    /* スパース形式コピー */
    int *ia = new int[n+1];
    auto row_ptr = matA.matrix->matrix.outerIndexPtr();
