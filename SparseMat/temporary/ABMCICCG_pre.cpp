#include "MatSolvers_ABMCICCG.hpp"
namespace SRLfem
{
    void ABMCICCG::sortAlgebraicMultiColor(int size0, const SparseMatBaseD &A, SparseMatBaseD &PA, const double *b, double *Pb, std::vector<std::vector<int>> &color_list, std::vector<int> &ordering, std::vector<int> &back_ordering, int num_colors)
    {
        ...
        for (int i = 0; i < color_list.size(); i++)
        {
            for (int j = 0; j < color_list[i].size(); j++)
            {
                ...
                back_ordering[k] = color_list[i][j];
                color_list[i][j] = k;
                k++;
            }
        }
    ...
    void ABMCICCG::makeAlgebraicBlock(int size0, const SparseMatBaseD &A,
                                      SparseMatBaseD &Mb,
    ...
    void ABMCICCG::sortAlgebraicBlockMultiColor(int size0,
                                                ...
                                                int num_colors)
    {
        ...
        std::vector<std::vector<int>> back_block_list;
        auto start = std::chrono::high_resolution_clock::now(); // record start time
        makeAlgebraicBlock(numRow, A, Mb, back_block_list, num_blocks);
        ...
        std::vector<int> back_block_ordering;
        start = std::chrono::high_resolution_clock::now(); // record start time
        sortAlgebraicMultiColor(max_block, Mb, PMb, y, z, color_list, block_ordering, back_block_ordering, num_colors);
        ...
        // back_block_ordering: new block index -> old block index
        delete[] y;
        delete[] z;
        int k = 0;
        ...
        // size of block_list is equivalent to back_block_list
        block_list.resize(back_block_list.size());

        start = std::chrono::high_resolution_clock::now(); // record start time

        // make block_list: new block index order
        for (int i = 0; i < back_block_ordering.size(); i++)
        {
            int bl = back_block_list[back_block_ordering[i]].size();
            block_list[i].resize(bl);

            for (int j = 0; j < bl; j++)
            {
                ordering[back_block_list[back_block_ordering[i]][j]] = k;
                back_ordering[k] = back_block_list[back_block_ordering[i]][j];
                block_list[i][j] = k;
                k++;
            }
        }
    ...
    bool ABMCICCG::parallelIccgSolv(int size0,
                                    const SparseMatBaseD &A,
...
#pragma omp parallel for
    ...
    /* *********************************************************** */
    void ABMCICCG::parallelIcSolv(int size0, const SparseMatBaseD &L, const SparseMatBaseD &Lt, std::vector<std::vector<int>> &block_list, std::vector<std::vector<int>> &color_list,
...
#pragma omp parallel for private(bl, l, nu, t, idx)
    ...
    bool ABMCICCG::solveICCGwithABMC(const slv_int size0, const double conv_cri, const int max_ite, const double accera, const double normB, const SparseMat &matA, double *vec_b, double *vec_x, int num_blocks, int num_colors)
    {
        bool is_conv = false;
        ...
        auto start = std::chrono::high_resolution_clock::now(); // record start time
        ABMCICCG::sortAlgebraicBlockMultiColor(n, *(matA.matrix), *(PA.matrix), vec_b, vec_Pb, block_list, color_list, ordering, back_ordering, num_blocks, num_colors);
        auto end = std::chrono::high_resolution_clock::now();             // record stop time
        std::chrono::duration<double, std::milli> duration = end - start; // calculate duration (ms)
        ...
        start = std::chrono::high_resolution_clock::now(); // record start time
        is_conv = ABMCICCG::parallelIccgSolv(n, *(PA.matrix), *(matL.matrix), *(matL_tr.matrix), block_list, color_list, diagD, vec_Pb, vec_x, epsilon, max_ite);
        end = std::chrono::high_resolution_clock::now(); // record stop time
        duration = end - start;                          // calculate duration (ms)
        ...
        for (int i = 0; i < n; i++)
        {
            vec_x[back_ordering[i]] = vec_r[i];
            normX += vec_x[back_ordering[i]] * vec_x[back_ordering[i]];
        }
