#include "ABMCICCG.hpp"
namespace SRLfem
{
    void ABMCICCG::sortAlgebraicMultiColor(int size0, const SparseMatBaseD &A, std::vector<std::vector<int>> &color_list, std::vector<int> &ordering, std::vector<int> &reverse_ordering, int num_colors)
    {
    ...
    void ABMCICCG::makeAlgebraicBlock(int size0,
                                                ...
                                                int num_colors)
    {
        ...
        sortAlgebraicMultiColor(max_block, Mb, color_list, block_ordering, reverse_block_ordering, num_colors);
...
#pragma omp parallel for
            ...
            /***solve [L][D][Lt]{ru} = {r}***/
            parallelIcSolv(size0, L, Lt, block_list, color_list, reverse_ordering, diagD, r, ru);
...
#pragma omp parallel for
    ...
    /* *********************************************************** */
    void ABMCICCG::parallelIcSolv(int size0,
                                  const SparseMatBaseD &L,
                                  const SparseMatBaseD &Lt,
                                  std::vector<std::vector<int>> &block_list,
                                  std::vector<std::vector<int>> &color_list,
...
#pragma omp parallel for private(bl, l, mu, t, idx)
                for (k = 0; k < color_list[col].size(); k++)
                {
                    ...
                    {
                        idx = reverse_ordering[block_list[bl][l]];
                        t = b(idx);
                        for (mu = start_pos[idx]; mu < end_pos[idx] - 1; mu++)
                        { // numNZ-1 is the diagnal !!
                            t -= val_ptr[mu] * y[reverse_ordering[col_ptr[mu]]];
                        }
...
#pragma omp parallel for private(bl, l, nu, t, idx)
                for (k = color_list[col].size() - 1; k >= 0; k--)
                {
                    ...
                    for (l = block_list[bl].size() - 1; l >= 0; l--)
                    {
                        ...
                        for (nu = start_pos2[idx] + 1; nu < end_pos2[idx]; nu++)
                        {
                            ...
                            t -= val_ptr2[nu] * x(reverse_ordering[col_ptr2[nu]]);
                        }
    ...
    bool ABMCICCG::solveICCGwithABMC(const slv_int size0,
                                     ...
                                     int num_colors)
    {
        ...
        auto start = std::chrono::high_resolution_clock::now(); // record start time
        ABMCICCG::sortAlgebraicBlockMultiColor(n, *(matA.matrix), block_list, color_list, ordering, reverse_ordering, num_blocks, num_colors);
        auto end = std::chrono::high_resolution_clock::now();             // record stop time
        std::chrono::duration<double, std::milli> duration = end - start; // calculate duration (ms)
        ...
        start = std::chrono::high_resolution_clock::now(); // record start time
        is_conv = ABMCICCG::parallelIccgSolv(n, *(matA.matrix), *(matL.matrix), *(matL_tr.matrix), block_list, color_list, reverse_ordering, diagD, vec_b, vec_x, epsilon, max_ite);
        end = std::chrono::high_resolution_clock::now(); // record stop time
        duration = end - start;                          // calculate duration (ms)
